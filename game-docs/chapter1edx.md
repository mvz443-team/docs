...
第一次到达80% ， 60%， 40% ， 20%血量，分别释放一次技能0

第一次到达50%血量， 场景闪耀一下， 同时重新开启自动产能
...
向 col > 4的任意 1(col)x2(row)位置放置一个 矿石Obstacle，这个Obstacle用从上向下的坠落动画出现，并且要有starshards环绕的效果。出现时，将该2格的植物全部压死。死亡后，为all player添加1个星之碎片

这个矿石占据 1x2 的大小

1. 盾构机

boss从当前位置向前冲刺5 col（最远冲刺到col=2）， 对当前row的植物全部压扁，对上下row的植物造成100伤害

2. 飞天蜈蚣

在随机3个不同的row的maxCol+1位置，分别召唤一个召唤的怪物 蜈蚣，该蜈蚣速度为20，只会一直前进，并压扁路上的一切植物。 蜈蚣血量为500.

3. 矿山爆破

向  2 < col < 6 的任意2个格子分别抛射（arc动画） 1个引燃的tnt ， 这个tnt落地后产生爆炸效果，对3x3范围造成500点爆炸伤害
